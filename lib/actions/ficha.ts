...
export async function getFichaCompleta(alumnoId: string) {
  ...
  const [alumno, asistAnio, asistMes, notas, evals, anotaciones, derivaciones, entrevistas] =
    await Promise.all([
      supabase.from('alumnos')
        .select('*, cursos(nombre, nivel, anio)')
  ...
  for (const asig of Object.values(asignaturas)) {
    asig.prom = asig.notas.length > 0
      ? Math.round(asig.notas.reduce((a,b) => a+b,0) / asig.notas.length * 10) / 10 : null
  }

  const todasNotas = notasData.map(n => n.nota).filter(Boolean) as number[]
  const promGeneral = todasNotas.length > 0
    ? Math.round(todasNotas.reduce((a,b) => a+b,0) / todasNotas.length * 10) / 10 : null
...
export async function crearAnotacion(params: {
  ...
  const { data: perfil } = await supabase
    .from('perfiles').select('establecimiento_id').eq('id', user.id).single()
...
export async function actualizarAlumno(alumnoId: string, datos: Record<string, any>) {
  const supabase = await createClient()
  const { error } = await supabase.from('alumnos').update(datos).eq('id', alumnoId)
  if (error) return { error: error.message, success: false }
  return { success: true, error: null }
}
