...
export async function guardarColegio(
  ...
  if (existing) {
    return {
      error:
        'Este RBD ya está registrado en Kiva360. Contacta soporte si crees que es un error.',
      success: false,
      field: 'rbd',
    }
  }
...
export async function guardarRol(
  ...
  if (!parsed.success) {
    return {
      error: 'Selecciona un rol para continuar',
      success: false,
    }
  }
  ...
  if (!establecimiento_id) {
    return {
      error: 'Error de sesión. Vuelve al paso anterior.',
      success: false,
    }
  }
